pipeline {
  environment {
    imagename = "kristosako/spring-quiz"
  ...
  stages {
    stage('Cloning Git') {
      steps {
        git([url: 'https://github.com/krsako/quiz-backend.git', branch: 'main', credentialsId: 'github'])
      }
    }
    stage('Build Image') {
      steps{
    ...
    stage('Deploy image to AKS-Test') {
      steps{
          withCredentials([usernamePassword(credentialsId: 'azure', usernameVariable: 'USERNAME', passwordVariable: 'PASSWORD')]) {
            ...
            sh "kubectl set image deployment/quiz-client quiz-server=${imagename}:${BUILD_ID}"
        }
      }
    }
  }
